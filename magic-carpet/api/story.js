...
async function advanceSouls(world, state, beats, key) {
  ...
  const sys = '你是一部互动小说的"暗线推演引擎"。依据每个角色的欲望/价值观/秘密，推演他们此刻的抉择。每个角色的"浮现后果"必须与玩家刚才的动作直接相关，不要凭空冒出。只输出 JSON，不要任何解释。';
  const user =
`世界基调：${world.tone}
玩家刚才：${lastMove}
需要推进的暗线节拍：
...
// 叙事连贯：把"故事梗概 + 上一段旁白原文 + 玩家本回合动作"回传给下一次生成
function continuityBlock(state, playerAction, first) {
  let b = '';
  if (state && state.summary) b += `【故事梗概（至今，供衔接）】\n${state.summary}\n`;
  if (state && state.last_narration) b += `【上一段旁白原文——请紧接它自然续写，不要重述、不要跳】\n${state.last_narration}\n`;
  if (!first && playerAction) b += `【玩家本回合动作——请先直接、即时地回应它，让后续由它长出来】\n${playerAction}\n`;
  return b;
}
...
async function narrate(world, state, opts, key) {
  ...
  const cont = continuityBlock(state, opts.playerAction, opts.first);
...
- 必须紧接给你的"上一段旁白"自然续写，并先直接回应玩家本回合的动作；不重述、不跳跃、不另起炉灶。`;
...
${cont}最近发生：\n${recent}
请紧接上一段旁白，只写一段收束全篇的结尾旁白（不要选项、不要标题）。
然后另起一行写三个等号：===
再写一行：SUMMARY: 用一句话概括整段故事的结局`;
  } else {
    user =
...
最后另起一行写三个等号：===
再写一行：SUMMARY: 用一句话概括本回合发生了什么（供后续衔接）`;
  }
  ...
  const seg0 = raw.split(/\n\s*={3,}\s*\n/);
  const mainR = (seg0[0] || raw).trim(); const metaR = seg0[1] || '';
  const recap = ((metaR.match(/SUMMARY\s*[:：]\s*(.+)/i) || [])[1] || '').trim();

  if (!wantChoices) return { narration: mainR || '（夜色沉沉，故事在这里静静落幕。）', choices: [], recap };

  const parts = mainR.split(/\n\s*-{3,}\s*\n/);
  let narration = (parts[0] || mainR).trim();
  let choices = [];
  if (parts[1]) {
  ...
  return { narration, choices, recap };
}
...
- 必须紧接给你的"上一段旁白"续写，并先直接回应玩家本回合的动作；不重述、不跳跃。`;
  const user =
`玩家：${world.player_role.who}
...
再加一行：SUMMARY: 用一句话概括本回合发生了什么`;
  ...
  const recap = ((metaPart.match(/SUMMARY\s*[:：]\s*(.+)/i) || [])[1] || '').trim();
  return { narration, choices, status, discover, recap };
}
...
- 必须紧接给你的"上一段旁白"续写，并先直接回应玩家本回合的动作；不重述、不跳跃、不另起炉灶。`;
  const user =
`玩家：${world.player_role.who}
...
再加一行：SUMMARY: 用一句话概括本回合发生了什么`;
  ...
  const recap = ((metaPart.match(/SUMMARY\s*[:：]\s*(.+)/i) || [])[1] || '').trim();
  return { narration, choices, clues, unlock, status, ending, recap };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (world.mode === 'deduction') {
      ...
      const outX = await narrateDeduction(world, state, { surfaced, first, playerAction: choice }, anthKey);
      ...
      state.last_narration = outX.narration; if (outX.recap) state.summary = ((state.summary ? state.summary + ' ' : '') + outX.recap).slice(-700);
    ...
    if (world.mode === 'dungeon') {
      const outD = await narrateDungeon(world, state, { surfaced, first, playerAction: choice }, anthKey);
      (outD.discover || []).forEach(id => { if (world.rules.some(r => r.id === id) && !state.discovered.includes(id)) state.discovered.push(id); });
      state.last_narration = outD.narration; if (outD.recap) state.summary = ((state.summary ? state.summary + ' ' : '') + outD.recap).slice(-700);
    ...
    const out = await narrate(world, state, { surfaced, first, ending, playerAction: choice }, anthKey);
    state.last_narration = out.narration; if (out.recap) state.summary = ((state.summary ? state.summary + ' ' : '') + out.recap).slice(-700);
